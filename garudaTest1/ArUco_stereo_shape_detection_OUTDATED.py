...
def perform_shape_detection(frame1, frame2):
    ...
    # Detectar marcadores en ambas cámaras
    mds1.detect_markers(frame1)
    mds2.detect_markers(frame2)

    # Dibujar detecciones en los frames (para depuración visual)
    mds1.draw_markers(frame1)
    mds2.draw_markers(frame2)

    # Imprimir IDs detectadas en consola
    print("Cam1 IDs:", list(mds1.detected_markers.keys()))
    print("Cam2 IDs:", list(mds2.detected_markers.keys()))

    # Obtener centroides ordenados (por ID)
    corners1 = mds1.centroids_sorted
    corners2 = mds2.centroids_sorted

    height, width, depth = frame1.shape

    corner_3d_positions = []

    # Solo calcular si ambas cámaras tienen detecciones
    if corners1 and corners2:
        ...
        print("No se encontraron marcadores en una o ambas cámaras.")

    # Crear vista cenital si hay puntos
    if corner_3d_positions:
        birds_eye_view = create_birds_eye_view(corner_3d_positions)
    else:
        birds_eye_view = None

    # No usamos contornos, pero devolvemos listas vacías para no romper el flujo
    contours1 = []
    contours2 = []

    # Convertir centroides a enteros para dibujo posterior
    corners1_int = [tuple(map(int, pt)) for pt in corners1]
    corners2_int = [tuple(map(int, pt)) for pt in corners2]

    return contours1, corners1_int, contours2, corners2_int
...
while True:
    ...
    if current_phase >= PHASE_SHAPE_DETECTION and shape_detection_done:
        ...
        for i, corner in enumerate(corners1):
            cv2.circle(display_frame1, corner, 5, (0, 0, 255), -1)
            cv2.putText(display_frame1, f"{i}", corner, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

        for i, corner in enumerate(corners2):
            cv2.circle(display_frame2, corner, 5, (0, 0, 255), -1)
            cv2.putText(display_frame2, f"{i}", corner, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
