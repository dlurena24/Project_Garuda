...
def create_birds_eye_view(corner_3d_positions, object_positions=None, canvas_size=(600, 600), margin=50):
    ...
    # Include object positions in range calculation if available
    if object_positions:
        for obj_pos in object_positions:
            x, z = obj_pos[0], obj_pos[1]
            x_coords.append(x)
            z_coords.append(z)
    ...
    # Add object markers if detected
    if object_positions:
        # Define colors for different object classes (can be expanded)
        class_colors = [
            (0, 255, 0),  # Green
            (0, 0, 255),  # Red
            (255, 0, 0),  # Blue
        ...
        for obj_pos in object_positions:
            x, z, cls_id, conf = obj_pos
            obj_canvas_point = transform_point(x, z)

            # Get color based on class ID
            color = class_colors[cls_id % len(class_colors)]

            # Draw object marker
            cv2.circle(canvas, obj_canvas_point, 8, color, -1)
            ...
            cv2.putText(canvas, label_text, (obj_canvas_point[0] + 10, obj_canvas_point[1]),
...
def perform_object_detection(frame1, frame2):
    global object_positions, birds_eye_view

    height, width, depth = frame1.shape

    # Object detection
    left_objects = calculate_obj_coordinates(frame1)
    right_objects = calculate_obj_coordinates(frame2)

    # Reset for new detections
    object_positions = []

    # Match objects between cameras (simple approach - can be improved)
    # This is a simplified matching that assumes objects are in similar positions in both frames
    for left_obj in left_objects:
        xL, yL, cls_id_L, conf_L = left_obj

        # Find closest matching object in right camera
        best_match = None
        min_distance = float('inf')

        for right_obj in right_objects:
            xR, yR, cls_id_R, conf_R = right_obj

            # Only match objects of the same class
            if cls_id_L == cls_id_R:
                # Simple distance metric (can be improved)
                dist = abs(yL - yR)  # Assuming similar vertical position
                if dist < min_distance:
                    min_distance = dist
                    best_match = right_obj

        # If we found a match within reasonable distance
        if best_match and min_distance < 50:  # Threshold can be adjusted
            xR, yR, cls_id_R, conf_R = best_match

            resultOBJ = calculate_3d_point(xL, xR, width)
            if resultOBJ:
                X, Z = resultOBJ
                # Store object position with class ID and confidence
                object_positions.append((X, Z, cls_id_L, conf_L))

    # Update bird's eye view with all object positions
    if corner_3d_positions:
        birds_eye_view = create_birds_eye_view(corner_3d_positions, object_positions)
...
# Initialize webcams
cap1 = cv2.VideoCapture(1)  # Left camera
cap2 = cv2.VideoCapture(0)  # Right camera
...
while True:
    ...
    if current_phase == PHASE_SHAPE_DETECTION and not shape_detection_done:
        ...
        if not object_positions or (current_time - last_object_scan_time) >= 5:
            left_objects, right_objects = perform_object_detection(frame1, frame2)
            last_object_scan_time = current_time
            if object_positions:
                print(f"Detected {len(object_positions)} objects")
    ...
    if current_phase == PHASE_OBJECT_DETECTION and object_positions:
        if left_objects:
            for obj in left_objects:
                x, y, cls_id, conf = obj
                # Get color based on class ID
                class_colors = [
                    (0, 255, 255),  # Yellow (default)
                    (0, 255, 0),    # Green
                    (0, 0, 255),    # Red
                    (255, 0, 0),    # Blue
                    (255, 0, 255),  # Magenta
                ...
                cv2.circle(display_frame1, (x, y), 10, color, -1)
                ...
                cv2.putText(display_frame1, label_text, (x + 10, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        ...
        if right_objects:
            for obj in right_objects:
                ...
                cv2.circle(display_frame2, (x, y), 10, color, -1)
                ...
                cv2.putText(display_frame2, label_text, (x + 10, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            cv2.circle(display_frame2, tuple(corner), 5, (0, 0, 255), -1)
            cv2.putText(display_frame2, f"{i}", tuple(corner), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

    # Draw object markers if detected
    if current_phase == PHASE_OBJECT_DETECTION and object_positions is not None:
        if xL is not None:
    ...
    if key == ord('q'):  # Quit
        ...
        if current_phase == PHASE_SHAPE_DETECTION and shape_detection_done:
            ...
            object_positions = []  # Reset to trigger new object detection
